...
<%
    ...
    try (Connection con = DriverManager.getConnection(urlForLoadData, uid, pw);) {
        ...
        // Validate customerId
        if (!rst.isBeforeFirst() || !custId.chars().allMatch(Character::isDigit)) {
            out.print("<p style='color: red;'>Error: Incorrect customerId. Please try again.</p>");
            valid = false;
        }
    } catch (Exception e) {
    ...
    // Validate password
    if (password == null) {
        out.print("<p style='color: red;'>Error: Password cannot be null. Please try again.</p>");
        valid = false;
    } else {
        try (Connection con = DriverManager.getConnection(urlForLoadData, uid, pw);) {
            ...
            if (rst.next()) {
                ...
                if (!password.equals(storedPassword)) {
                    out.print("<p style='color: red;'>Error: Incorrect password. Please try again.</p>");
                    valid = false;
                }
            } else {
                out.print("<p style='color: red;'>Error: Customer ID not found. Please try again.</p>");
                valid = false;
            }
        } catch (Exception e) {
    ...
    if (valid) {
        try (Connection con = DriverManager.getConnection(urlForLoadData, uid, pw);) {
            ...
            PreparedStatement pstmt = con.prepareStatement("INSERT INTO ordersummary (orderDate, customerId) VALUES (?, ?)", Statement.RETURN_GENERATED_KEYS);
            ...
            pstmt = con.prepareStatement("UPDATE ordersummary SET totalAmount = ? WHERE orderId = ?");
    ...
    // Back to shop button
    out.println("<form action='shop.html'><input type='submit' value='Back to Shop'></form>");
%>
